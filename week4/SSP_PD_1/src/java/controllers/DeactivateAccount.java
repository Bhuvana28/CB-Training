...
public class DeactivateAccount extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DeactivateAccount</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DeactivateAccount at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
            ...
            if(userId!=null){
                try{
                    ...
                    for(Contact contact:contacts){
                        phoneNumbersDao.deleteContactPhoneNumbers(contact.getContactId());
                        contactsDao.deleteContactInfo(contact.getContactId());
                        if(contact.getContactAddressId()!=null){
                            addressesDao.deleteAddress(contact.getContactAddressId());
                        }
                    }
                    usersDao.deleteUser(userId);    
                    if(user.getAddressId()!=null){
                        addressesDao.deleteAddress(user.getAddressId());
                    }
